...
虛擬機運行環境系統：Ubuntu 18.04.6；

虛擬機運行環境系統：RAM 4G，ROM 50G，6核CPU ；

操作權限：使用超級管理員root權限，設定方法見附錄3；

docker安裝檔案：fr_docker.tar.gz；
...
若使用者已安裝部署docker，則跳過此節，進行1.3鏡像部署。

1. 下載fr_docker.tar.gz，放至Ubuntu檔案路徑/opt/。

2. 解壓fr_docker.tar.gz.，以/opt/目錄下為例：
...
1. 下載虛擬機鏡像FAIRINOSimMachine.tar並解壓。
...
出現fairno_simmachine:latest則表示導入完成。

.. image:: controller_virtual_machine/039.png
   :width: 6in
   :align: center

4. 執行docker images查看是否導入成功。

創建自定義橋接網路
++++++++++++++++++++

1. 執行以下命令，創建名為fairino-net，網段為192.168.58.0/24的橋接網路。
...
使用fairino-net網路，fairino_simmachine鏡像啟動容器。
...
此小節針對非首次啟動容器，由於重啟電腦或docker關閉等原因容器未在後台運行情況。
...
出現fairino-container則表示正在運行。

.. image:: controller_virtual_machine/047.png
   :width: 6in
   :align: center

2. 打開瀏覽器，輸入預設IP：192.168.58.2，即可訪問web介面，操作虛擬機器人。

.. image:: controller_virtual_machine/048.png
   :width: 6in
   :align: center

3. 使用admin帳號登入，密碼：123。
...
1. 打開瀏覽器，輸入預設 IP： 192.168.58.2，打開 web 頁面；
2. 使用 admin 帳號登入，密碼： 123；
3. 進入「系統設定」 → 「通用設定」 → 「網路設定」， 修改 IP 為目標 IP 地址、遮罩、閘道。點擊「設定網路」；
4. 打開終端，關閉容器；
...
.. code-block:: console
   ...
   docker network create --driver bridge --subnet [目標IP/子網路遮罩] --gateway [閘道IP] fairino-net
...
8. 此時打開瀏覽器， 輸入修改後 IP 地址，即可訪問 web 介面，操作虛擬機器人。
...
1. 已部署及正常使用的FAIRINO SimMachine Docker虛擬機。部署教程見《使用者手冊-Linux部署docker鏡像》；
