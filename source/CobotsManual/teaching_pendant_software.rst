...
- **Step6示教程式設計**：點選新建，選擇空白模板，點選進入PTP功能程式設計項，選擇剛儲存的初始位置點，點選「新增」按鈕，套用完成後，在程式檔案中會顯示一條PTP指令；然後點選進入TPD功能編程項，選擇剛剛記錄的軌跡，設定是否平滑以及速度縮放比例，點擊「新增」按鈕，應用完成後，在程式檔案中會顯示一條MoveTPD指令，如下圖所示；
