...
通過關節扭矩感測器對外力進行即時檢測，當外力超出預設閾值時，機械臂會沿外力施加方向拖動；一旦外力低於力矩閾值以下，機械臂隨即停止拖動，從而實現位置環控制下的拖動示教功能。
...
位置環拖動示教功能啟停
""""""""""""""""""""""""""""""""""""""""""""""""""""""""
**Step1**：設置工具座標系為「Tool0」，點擊「輔助應用」->「工具應用」->「拖動鎖定」，在「關節扭矩感測器整機拖動模組」中點擊「功能啟用」。

.. image:: application/042.png
   :width: 4in
   :align: center

.. centered:: 圖表 14.5‑9 功能啟用

**Step2**：進行零點標定，先點擊「點位1」，等待機械臂運動停止後點擊「記錄」；再點擊「點位2」，等待機械臂運動停止後點擊「記錄」。如果零點標定完成後關節扭矩感測器零點偏差過大，可重新進行零點標定。

**Step3**：進行靈敏度標定，點擊「生成程式」下發控制器內部Lua腳本，將機器人切換為自動模式並設置運行速度為「10」，點擊「運行」，等待機器人運動。待機器人運動完成後，靈敏度、線性度、遲滯誤差及重複性標定結果自動顯示至Web介面。靈敏度標定在機械臂每次上電時僅需執行一次。

.. image:: application/043.png
   :width: 4in
   :align: center

.. centered:: 圖表 14.5‑10 零點標定及靈敏度標定

**Step4**：進入「拖動功能」，在拖動模式下拉框中選擇「位置環拖動」，並設置合適的質量系數、阻尼系數、剛度系數以及力控制閾值，點擊「設置」應用，即可開啟位置環拖動。

.. image:: application/044.png
   :width: 4in
   :align: center

.. centered:: 圖表 14.5‑11 位置環拖動參數設定

**Step5**：在拖動模式下拉框中選擇「電流環拖動」，點擊「設置」應用，即可關閉位置環拖動，切換為電流環拖動。

.. image:: application/045.png
   :width: 4in
   :align: center

.. centered:: 圖表 14.5‑12 電流環拖動

**Step6**：若在位置環拖動模式下出現未施加外力時機械臂仍發生移動的現象，可適當增大力控制閾值。如果單軸力控制閾值增大至3Nm以上後，該現象仍未解決，則需要關閉位置環拖動，並重新執行Step2和Step4（無需重新執行Step3）。

參數具體作用及推薦值：

- **質量系數**：用於調節拖動響應，數值越大響應越慢，數值越小響應越快。1~2關節範圍為[0.5-20]，3~4關節範圍為[0.2-20]，5~6關節範圍為[0.2-10]，推薦參數為[1, 0.8, 0.5, 0.5, 0.6, 0.6]；
- **阻尼系數**：用於調節拖動手感輕重，數值越大手感越重，數值越小手感越輕。1~2關節範圍為[5-100]，3~4關節範圍為[5-100]，5~6關節範圍為[2-40]，推薦參數為[8, 8, 10, 10, 6, 6]；
- **剛度系數**：設置此參數後，機械臂移動到功能開啟時的參考位置，推薦參數為[0, 0, 0, 0, 0, 0]；
- **力控制閾值**：位置環拖動的最小觸發力。力控制閾值設置過小，可能導致機械臂非預期移動。1關節推薦力閾值大於2Nm，2關節推薦力閾值大於1Nm，3~4關節推薦力閾值大於0.8Nm，5~6關節推薦力閾值大於0.5Nm，推薦參數為[2, 1, 0.8, 0.8, 0.5, 0.5]。
