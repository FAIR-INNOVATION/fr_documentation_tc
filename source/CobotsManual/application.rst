...
1. 升級前，請在《系統設定-關於》中查看並確認當前軟件版本。
2. 軟件升級包下載地址見法奧文件《資料下載-機器人軟件下載》，解壓後包含對應版本的升級包 ``software.tar.gz``。

注意事項
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

1. **數據備份**：建議升級前執行備份（參考3.2.1節），避免因升級異常導致數據丟失。
2. **版本限制**：

.. centered:: 表 14.1-1 版本升級限制
...
3. **緩存清除**：每次升級後（特別是跨版本升級），建議清除瀏覽器緩存以確保系統正常運行。
...
1. 進入「輔助應用」->「工具應用」菜單下的軟件升級頁面。

.. image:: application/002.png
   :width: 3in
   :align: center

.. centered:: 圖表 14.2‑1 系統升級界面

1. 點擊「選擇文件」，選擇官網下載的 'software.tar.gz' 升級包。

.. important:: 
   升級包文件名必須為 'software.tar.gz'，若不一致將導致升級失敗，請修改文件名。

3. 點擊「上傳升級包」開始升級，進度條將顯示升級狀態。

4. 進度達100%時，界面提示「升級成功，請重啟控制箱」。

.. image:: application/082.png
   :width: 3in
   :align: center

.. centered:: 圖表 14.2‑2 軟件升級成功

5. 重啟控制箱後，在「關於」中確認版本信息。

**固件升級**：機器人進入BOOT模式後，上傳升級包，選擇需升級的從站（控制箱從站、本體驅動器從站1~6、末端從站），並顯示升級狀態。

.. image:: application/003.png
   :width: 3in
   :align: center

.. centered:: 圖表 14.2‑3 固件升級

**從站配置文件升級**：機器人去使能後，上傳升級文件，選擇目標從站（控制箱從站、本體驅動器從站1~6、末端從站），並顯示升級狀態。

.. image:: application/004.png
   :width: 3in
   :align: center

.. centered:: 圖表 14.2‑4 從站配置文件升級
...
**步驟1**：點擊「輔助應用」->「工具應用」->「拖動鎖定」，進入力感測器輔助鎖定配置界面，設定「狀態開關」、「碰撞檢測」為開啟，如下圖所示。

.. image:: application/080.png
   :width: 4in
   :align: center

.. centered:: 圖表 14.10‑2 配置力感測器輔助鎖定

**步驟2**：拖動機器人，在機器人運動過程中向關節處施加外力，觸發碰撞防護，web界面報錯「力感測器輔助拖動碰撞故障」，並可通過web界面快速恢復/關閉力感測器輔助拖動，如圖所示。點擊「恢復」，清除報錯並恢復開啟力感測器輔助拖動；點擊「關閉」，清除報錯並保持關閉力感測器輔助拖動。

.. image:: application/081.png
   :width: 3in
   :align: center

.. centered:: 圖表 14.10‑2 力感測器輔助拖動下觸發碰撞

.. note:: 在力感測器輔助拖動下，機器人本身處於停止狀態，在拖動時關節轉矩指令與反饋存在差值，建議設定碰撞等級為7級及以上，碰撞等級設定過小會出現拖動時誤報碰撞錯誤。
...
CAD軟體生成G代碼轉機器人軌跡規劃功能是使用CAD軟體把直線、圓弧、整圓、樣條線路徑轉成副檔名為「.gcode」的G代碼文件，其中樣條線路徑的G代碼是由若干個小直線代碼組成的，將生成的G代碼文件在Web端導入便能轉化生成LUA文件。

功能說明：

(1) Web端只能導入副檔名為「.gcode」的G代碼文件，轉化成功後會生成與G代碼文件同名的LUA文件。如果轉化前已經有同名的LUA文件，轉化會失敗。

(2) 目前能夠對G代碼中的快速移動G0、直線插補G1、順時針圓弧插補G2、逆時針圓弧插補G3指令進行轉化，其中G0對應MoveJ指令、G1對應MoveL指令、圓弧的G2/G3對應MoveC指令，整圓的G2/G3對應Circle指令。

(3) 目前僅能對XY平面的圓弧和圓路徑G代碼進行轉化。

(4) G代碼指令中設置主軸轉速的S對應MoveJ指令中的速度，主軸轉速的單位為轉/分鐘，對應移動速度的毫米/分鐘，進給速度F對應MoveL、MoveC、Circle中的速度。進給速度的單位為毫米/分鐘，與移動速度單位一致。G代碼中的速度設置不能超過機器人的最大移動速度。

(5) 機器人在執行轉化生成的LUA文件時，需要把Web界面右上角的速度百分比改為100。
...
.. centered:: 圖表 14.15-1 機器人姿態計算示意圖

其中，P-xyz為記錄的參考姿態示教點的姿態，O-xy為CAD圖紙的坐標系。機器人在起點A處的姿態是參考姿態，根據參考姿態Z軸與CAD圖紙平面的夾角以及Z軸在CAD圖紙平面的投影與路徑起點處切線的夾角，計算路徑中間點B和終點C處的機器人姿態。

操作步驟如下：

**步驟1**：使用具有CAM功能的CAD軟體，將加工路徑轉為G代碼文件，並且使用G代碼模擬器（如NC Viewer等）驗證刀具路徑是否正確。

**步驟2**：在G代碼轉機器人軌跡規劃之前，首先對工具坐標系和工件坐標系進行標定，注意工件坐標系需要與CAD軟體中機床坐標系重合。

.. image:: application/084.png
   :width: 6in
   :align: center

.. centered:: 圖表 14.15-2 工具和工件坐標系標定界面

**步驟3**：在標定後的工具坐標系和工件坐標系下記錄一個參考姿態示教點，機器人在工作路徑下的姿態會使用參考姿態點的姿態進行計算。

**步驟4**：依次點擊「輔助應用」、「工具應用」、「G代碼轉化」按鈕，進入G代碼文件轉機器人運動LUA文件界面。

.. image:: application/085.png
   :width: 6in
   :align: center

.. centered:: 圖表 14.15-3 G代碼轉化界面

**步驟5**：點擊「選擇文件」按鈕，找到需要轉化的G代碼文件（副檔名必須為「.gcode」）；參考姿態點選擇步驟2中記錄的參考姿態示教點，選擇成功後下方會顯示當前所選示教點的工具坐標系和工件坐標系。最後點擊「轉化」按鈕，轉化成功後會提示G代碼轉化成功。如果已有同名LUA文件，會提示文件名已存在。

.. image:: application/086.png
   :width: 6in
   :align: center

.. centered:: 圖表 14.15-4 G代碼轉化成功界面

.. image:: application/087.png
   :width: 6in
   :align: center

.. centered:: 圖表 14.15-5 G代碼轉化失敗界面

**步驟6**：點擊「示教編程」->「程序編程」按鈕，打開G代碼轉化後生成的LUA文件，機器人切換為自動模式，點擊開始按鈕，機器人便能復現G代碼文件裡的路徑。

.. image:: application/088.png
   :width: 6in
   :align: center

.. centered:: 圖表 14.15-6 執行轉化後的LUA文件