...
當安全停止的觸發方式設置為「雙通道」時，必須雙通道保證清除狀態並在操作介面人為清除警告後機器人才可復位。此外，在策略配置中增加縮減模式選項，當用戶選擇該策略時，機器人將進入縮減模式運動。

操作流程
++++++++++++++++++++++++++++++++++++

**Step1**：點擊「初始設置」—>「安全」—>「安全停止」按鈕，觸發方式可選為「默認」和「雙通道」兩種模式，二者不同之處在於：「默認」模式在觸發並恢復後自動清除介面報錯，「雙通道」模式在觸發並恢復後需手動清除介面報錯。「安全停止策略」可選擇「停止」、「暫停」、「一級縮減模式」和「二級縮減模式」等類型，詳細說明如下：當選為「停止」時，機器人將停止當前運動；當選為「暫停」時，機器人將暫停當前運動，恢復並清除報錯後，將恢復暫停；當選為「一級縮減模式」時，機器人將進入一級縮減模式運動；當選為「二級縮減模式」時，機器人將進入二級縮減模式運動。

.. image:: safety/048.png
   :width: 4in
   :align: center

.. centered:: 圖表 7.1-2 安全停止策略設置

**Step2**：因當觸發方式選為「默認」時，在觸發恢復後，可自動清除介面報錯，因此無需過多介紹。故主要介紹當觸發方式選為「雙通道」時的操作：在觸發恢復後，需手動點擊右上角的「清除」操作，機器人才可復位。

.. image:: safety/049.png
   :width: 4in
   :align: center

.. centered:: 圖表 7.1-3 手動清除安全停止觸發操作

安全速度移動
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
機器人在安全停止觸發後，可以在webapp中點擊按鈕、可配置控制箱CI輸入、可配置末端工具End DI輸入控制機器人進入安全速度移動狀態。在安全速度移動狀態中可以在安全速度下點動機器人，或進入拖動模式拖動機器人，以幫助用戶排除故障。

在機器人webapp中依次點擊「初始設置」、「安全」、「安全停止」，在該頁面中找到「安全速度移動」，並將它設置為使能。

.. image:: safety/057.png
   :width: 4in
   :align: center

.. centered:: 圖表 7.1-4 打開安全速度移動

此時觸發安全停止時，機器人webapp右上角顯示「安全停止觸發」警告，並顯示出「進入安全速度移動」按鈕。

.. image:: safety/058.png
   :width: 4in
   :align: center

.. centered:: 圖表 7.1-5 進入安全速度移動彈框

點擊「進入」按鈕，機器人將自動停止LUA程序，並切換至手動模式。同時「進入安全速度移動」按鈕更新顯示為「已進入」。此時通過末端按鈕、按鈕盒、webapp等方式控制機器人進入拖動模式拖動機器人，也可以通過webapp或示教器Jog點動機器人。

.. image:: safety/059.png
   :width: 4in
   :align: center

.. centered:: 圖表 7.1-6 進入安全速度移動

機器人安全安全速度移動，並在笛卡爾空間點動機器人時，機器人的最大移動速度為設置的安全速度，若當前機器人全局速度設置大於安全速度，機器人點動將自動降速為安全速度。安全速度的設置在「初始設置」、「安全」、「安全速度」中。

機器人安全停止後，除了可以在webapp右上角控制機器人進入安全速度外，還可以通過控制箱CI輸入、末端CI輸入進入。在webapp中依次點擊「初始設置」、「基礎」、「I/O設置」、「DI」。配置控制箱某個端口CI或末端工具End DI為「進入安全速度移動」。安全停止觸發後，只需要觸發配置端口的輸入信號，即可進入安全速度移動。

.. image:: safety/060.png
   :width: 6in
   :align: center

.. centered:: 圖表 7.1-7 通過按鈕進入安全速度移動

僅自動模式安全停止
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
當機器人啟用安全停止模式(CR認證、功能安全)，並且搭配示教器三位使能使用時，可以打開「僅自動模式下安全停止」，當機器人安全停止信號觸發時，可以在示教器上將機器人切換至手動模式，並在手動模式下Jog點動機器人或進入拖動模式拖動機器人，以幫助用戶排除故障。

在機器人webapp中依次點擊「初始設置」、「安全」、「安全停止」，在該頁面中找到「僅自動模式安全停止」，並將它設置為使能。

.. image:: safety/061.png
   :width: 4in
   :align: center

.. centered:: 圖表 7.1-8 僅自動模式安全停止開啟

當機器人未啟用安全停止模式(CR認證、功能安全)，或未使用示教器時，無法打開「僅自動模式安全停止」功能，此時webapp會報出錯誤提示「僅自動模式安全停止需打開示教器三位使能」。同時，當禁用安全停止模式或禁用示教器時，「僅自動模式安全停止」功能也將自動關閉。

.. image:: safety/062.png
   :width: 3in
   :align: center

.. centered:: 圖表 7.1-9 僅自動模式安全停止開啟報錯
...
機器人安全速度功能是在人機協作或動態環境中，通過主動限制機器人的運行速度，將動能與衝擊力控制在安全閾值內，從而在意外接觸時防止人員受傷，並有效保護設備與工件免受碰撞損害。

操作流程
+++++++++++++++++++++++++++++

**Step1**：點擊「初始設置」-「安全」-「安全速度」按鈕，進行安全速度參數的設置：主要包含「功能啟用」、「限制速度」、「超速後模式」三部分。

其中，功能啟用中可選擇「不啟用」、「手動模式啟用」和「所有模式啟用」三種類型；

限制速度中進行限制速度的設置，機器人的線速度達到該限制速度時，將根據「超速後模式」的設定參數進行處理。「超速後模式」可選擇「停止報警」、「自動限速」和「停止報警後去使能」三種模式，自動限速僅可在「手動模式啟用」中使用。

設置完成所需參數後，無需多餘操作，將按照設定參數對機器人的運動進行處理，參數設置如圖所示。

.. image:: safety/056.png
   :width: 4in
   :align: center

.. centered:: 圖表 7.2-2 安全速度參數設置
...
**Step1**：點擊「初始設置」—>「安全」—>「急停停機」按鈕，「停機類型」選為「1b類」，並根據實際需要設置「停機時間限制」和「停機距離限制」參數，「急停復位後使能策略」可選為「手動使能」或「自動使能」，如圖2-1所示。

.. image:: safety/046.png
   :width: 4in
   :align: center

.. centered:: 圖表 7.4-2 使能策略設置
...
立方體干涉功能支援同時定義並啟動多個獨立的立方體干涉區，每個干涉區在三維空間中的位置和尺寸可獨立設定。此外，每個干涉區配備單獨的CO觸發訊號輸出，能夠基於機器人的即時位置輸出對應的觸發訊號。

操作流程
+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

**Step1**：進行立方體干涉功能啟用和基礎設定。依序點選「初始設定」->「安全」->「干涉區」->「立方體干涉」指令，透過滑塊開關控制每個立方體干涉區是否開啟，並進行基礎設定。

其中，進入干涉區運動策略可選為「繼續運動」或「停止」，當選為「繼續運動」時，機器人進入干涉區後將提示警告但繼續運動；當選為「停止」時，機器人進入干涉區後將提示警告並停止運動。進入干涉區拖動策略可選為「不限制拖動」、「阻抗回呼」和「切換回手動模式」。

.. image:: safety/050.png
   :width: 4in
   :align: center

.. centered:: 圖表 7.6‑21 立方體開啟控制與基礎設定

**Step2**：進行立方體干涉區設定。可為每個干涉區ID設定不同的設定參數，需注意的是：

(1) 偵測方法需根據實際功能需求選擇「指令位置」或「回饋位置」；

(2) 當干涉區模式選為「範圍外干涉」時，僅對單個干涉區有效。

.. image:: safety/051.png
   :width: 4in
   :align: center

.. centered:: 圖表 7.6‑22 立方體干涉區設定

**Step3**：進行干涉區範圍設定。範圍設定可選擇「兩點法」或「中心點+邊長」的方式產生立方體干涉區。「兩點法」方式透過指定立方體的兩個對角頂點產生。「中心點+邊長」方式透過指定立方體的中心點和三條邊長產生。

.. image:: safety/052.png
   :width: 4in
   :align: center

.. centered:: 圖表 7.6‑23 「兩點法」產生干涉區

.. image:: safety/053.png
   :width: 4in
   :align: center

.. centered:: 圖表 7.6‑24 「中心點+邊長」產生干涉區

**Step4**：進行CO訊號設定。依序點選「初始設定」->「基礎」->「I/O設定」-「DO」指令，為每個立方體設定相應的CO輸出。

.. image:: safety/054.png
   :width: 4in
   :align: center

.. centered:: 圖表 7.6‑25 CO輸出設定

**Step5**：各立方體干涉區將根據設定的ID序號在機器人介面進行顯示，當機器人末端中心點進入到干涉區後，介面會顯示「進入干涉區」警告，對應的CO介面輸出訊號。

.. image:: safety/055.png
   :width: 4in
   :align: center

.. centered:: 圖表 7.6‑26 多立方體干涉區介面顯示
