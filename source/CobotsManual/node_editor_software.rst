...
DMP是一種軌跡模仿學習的方法，需要事先規劃參考軌跡。在指令編輯介面 ，選擇示教點作為新的起點，點選「新增」、「套用」後可儲存指令。 DMP具體路徑為以新的起點模仿參考軌跡的新軌跡。
...
選擇要進行自動轉換的工件座標系，點選「新增」、「套用」後可儲存指令，新增PTP、LIN指令時與Body相連可實現在該指令內部執行，工件座標系下點位自動轉換。
...
選擇要進行自動轉換的工具座標系，點選「新增」、「套用」後可儲存該指令，新增PTP、LIN指令時與Body相連可實現在該指令內部執行，工具座標係下點位自動轉換。
...
- 工具座標系名稱：toolcoord1 ~ toolcoord14(SetToolList,[0~14]), etoolcoord0 ~ etoolcoord14(SetExToolList, [0~14])
