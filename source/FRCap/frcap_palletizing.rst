...
在協作機器人WebApp中「系統設定－外掛設定」頁面，點選「匯入」按鈕，選擇碼垛FRCap插件包（名稱格式：插件包名稱+版本號.plugin，範例：碼垛機Palletizer-v0.0.0 .plugin）上傳。上傳成功後清單展示導入成功的碼垛FRCap插件包，包括插件啟動停止狀態、名稱、版本號、描述和作者等。操作列中可以對碼垛FRCap插件包進行「停用」、「啟用」和「刪除」。
...
第一次成功導入碼垛FRCap插件包後，插件包狀態為“已停用”，點擊“啟用”按鈕，啟用成功後，協作機器人WebApp的“輔助應用”模組增加碼垛FRCap插件包開始頁面（例如：碼垛機Palletizer-v0.0.0.plugin對應的頁面模組名稱為「碼垛機Palletizer」）。點擊「開始」按鈕進入首頁，查看目前已配置的碼垛配方，根據需求進行使用。
...
在配方編輯中，可選擇是否使用棧板工作站，如圖2-2。使用棧板工作站則對應棧板功能使用工作站PLC中I/O信號完成；若選擇無棧板工作站，則預設使用控制箱上I/O信號完成棧板功能。

.. image:: frcap_pictures/076.png
   :width: 4in
   :align: center

.. centered:: 圖表 10-3-1-1 配方編輯頁面

棧板功能I/O接線配置
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

（1）在選擇使用棧板工作站後，點擊擴展I/O配置，可根據對應功能及與PLC的I/O介面實際接線，自定義選擇棧板功能的I/O信號配置，如圖為棧板工作站預設接線配置。

.. image:: frcap_pictures/077.png
   :width: 4in
   :align: center

.. centered:: 圖表 10-3-1-2 棧板工作站預設接線配置

（2）若選擇不使用棧板工作站，則預設使用控制箱I/O信號，根據對應功能及與控制箱I/O介面的實際接線，自定義選擇棧板功能的I/O信號配置，如圖為無棧板工作站（使用控制箱I/O）預設接線配置。

.. image:: frcap_pictures/078.png
   :width: 4in
   :align: center

.. centered:: 圖表 10-3-1-3 無棧板工作站（控制箱I/O）預設接線配置

棧板功能I/O通訊測試
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

（1）選擇使用棧板工作站時，配置完成工作站棧板工作站擴展I/O信號後，可點擊「測試」，對所接線的I/O功能進行測試驗證；
 
.. image:: frcap_pictures/079.png
   :width: 4in
   :align: center

.. centered:: 圖表 10-3-1-4 棧板工作站I/O接線測試

（2）選擇無棧板工作站時，配置完成棧板功能對應的控制箱I/O信號後，可點擊「測試」，對所接線的I/O功能進行測試驗證；
 
.. image:: frcap_pictures/080.png
   :width: 4in
   :align: center

.. centered:: 圖表 10-3-1-5 無棧板工作站（控制箱I/O）接線測試
...
在箱子配置欄中點擊“編輯”進入“箱子配置”彈窗，設置箱子的“長”、“寬”、“高”、“負載”、“工件標籤朝向”，點擊“確認”按鈕完成箱子資訊配置；設定箱子的抓取點（保持抓取點在箱子的中心，吸盤底部與箱子接觸時呈現擠壓狀態），點擊「記錄」按鈕完成設定。
...
彈窗頭部選擇模式，在箱子操作區域選擇箱子以添加該模式下的箱子，可選擇一鍵添加，預設無箱子間隔鋪滿居中托盤。自訂設定箱子間隔，可以單個添加也可以批量添加，點擊「確認」完成模式資訊設定。當選擇的箱子高度不一致時，無法完成配置，並提示「箱子類型高度不一致，不可被添加在同一模式」。
...
程式生成
------------
在配方展示下方查看「程式生成」，根據配方及需求選擇配方。當左右工位都選擇配方時，需要選擇啟動優先順序；當只選擇左工位或右工位配方時，無需選擇啟動優先順序。輸入程式名稱後，點擊「生成」按鈕。

.. note:: 所有程式名稱以「palletizing」開頭，無需輸入「palletizing」，只需要輸入「_」以後的名稱。例如「palletizing_program」，輸入「program」即可。

.. image:: frcap_pictures/043.png
   :width: 4in
   :align: center

.. centered:: 圖表 10-4-1 程式生成——左右工位同時選擇配方

.. image:: frcap_pictures/081.png
   :width: 4in
   :align: center

.. centered:: 圖表 10-4-2 程式生成——左工位選擇配方，右工位不選擇配方

.. important::
    1. 如果左工位或右工位未選擇棧板堆疊配方，則代表該工位不啟用。
    2. 生成程式成功後，務必在程式示教中將所有子程式和主程式手動儲存。
    3. 拆垛程式以「de」開頭，例如棧板堆疊程式為「palletizing_program」，拆垛程式則為「depalletizing_program」。
    4. 當執行左右工位都配置配方的程式時，同時接收到左右工件到位訊號後，根據設定的優先順序進行工作。
...
(1) 左右工位選擇相同的配方；
(2) 左工位和右工位選擇不同配方，但是配方中配置的箱子抓取點位姿相同。

.. image:: frcap_pictures/082.png
   :width: 4in
   :align: center

.. centered:: 圖表 10-4-3 單取料點箱子抓取點

雙取料點程式
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
雙取料點程式配置，需要左工位和右工位選擇不同配方，且配方中配置的箱子抓取點位姿不同。

.. image:: frcap_pictures/083.png
   :width: 4in
   :align: center

.. centered:: 圖表 10-4-4 雙取料點箱子抓取點