...
.. code-block:: c++
    ...
    /**
    * @brief 獲取焊機控制模式
    * @param [out] mode 焊機控制模式;0-直流一元模式；1-脈衝一元模式；2-JOB模式；3-近控模式；4-分別模式；5-CC/CV模式；6-TIG；7-CMT
    * @return 錯誤碼
    */
...
.. code-block:: c++
    ...
    /**
     * @brief 獲取擴展DI功能配置
     * @param [out] DIConfig 擴展DI輸入配置；DIConfig[0]-焊機準備擴展DI端口；
...
.. code-block:: c++
    ...
    /**
     * @brief 獲取擴展DO功能配置
     * @param [out] DOConfig 擴展DO輸入配置；DOConfig[0]-焊機起弧擴展DO端口；
...
.. code-block:: c++
    ...
    * @param[in] max_waittime 最大等待時間，單位毫秒，預設10000
    * @return 錯誤碼
    */
...
.. code-block:: c++
    ...
    std::this_thread::sleep_for(std::chrono::milliseconds(500));
    // ---- Step 3: WeaveStart, 啟動擺動MoveL執行緒 ----
    ...
    weaveThread.detach(); // 背景運行
    std::this_thread::sleep_for(std::chrono::milliseconds(500)); // 等待運動開始
    // ---- Step 4: 調速測試(主執行緒，擺動MoveL背景運行) ----
    ...
    std::this_thread::sleep_for(std::chrono::milliseconds(5000));
    // ---- Step 5: SetWeaveOffsetRT偏移測試(主執行緒，擺動MoveL背景運行) ----
    ...
    // ---- Step 6: 等待擺動MoveL完成, WeaveEnd ----
    std::cout << "\nStep 6: Wait for weave MoveL, then WeaveEnd" << std::endl;
    // 由於使用了 detach，這裡需要等待 weaveRunning 變為 false
    ...
    std::cout << "============================================================" << std::endl;
    std::cout << " 擺動調速與實時偏移測試 完成" << std::endl;
    std::cout << "============================================================" << std::endl;
    }