...
.. csv-table:: 
    ...
    "描述", "逆運動學求解，笛卡爾空間包含擴展軸位置"
    "必選參數", "- ``type``: 0-絕對位姿(基座標系)，1-增量位姿(基座標系)，2-增量位姿(工具座標系)
    - ``desc_pos``: 笛卡爾位姿
    - ``exaxis``: 擴展軸位置
    - ``tool``: 工具號
    - ``workPiece``: 工件號
    - ``config -1``: 自動求解，0-7對應八組解"
    "預設參數", "無"
    "返回值", "- 錯誤碼 成功-0 失敗- errcode
    - ``joint_pos``：關節位置"
...
.. csv-table:: 
    ...
    - 錯誤碼 成功-0  失敗- errcode
    - ``coord``：座標系數值
    - ``type``：工具類型 (可選) 0-工具；1-感測器
    - ``install``：安裝位置 (可選) 0-機器人末端；1-機器人外部
    - ``toolID``：工具ID
    - ``loadNo``：負載編號
    "
...
.. csv-table:: 
    ...
    - 錯誤碼 成功-0  失敗- errcode
    - ``coord``：座標系數值
    - ``tcoord``：機器人末端安裝工件座標系位姿 (6個值: [x, y, z, rx, ry, rz])
    "
...
.. csv-table:: 
    ...
    - 錯誤碼 成功-0  失敗- errcode
    - ``coord``：座標系數值
    - ``axisCoordNum``：擴展軸號 (可選) bit0-bit3對應擴展軸1-擴展軸4；如值為3,對應應用擴展軸[1,2]
    - ``calibFlag``：標定標誌 (可選) 0-未標定；1-已標定
    "
